...
function dataRequest(iG, iR, iM){
  var req = data["groups"][iG]["routes"][iR]['methods'][iM]["dialog"][0]["request"]||{"name":""};
  var resp = data["groups"][iG]["routes"][iR]['methods'][iM]["dialog"][0]["responses"][0];
  var url = data["groups"][iG]["routes"][iR]["route"];
  var iStart,iEnd,segment,replace,param,params,id;
  while (url.indexOf("{")!=-1){
    iStart = url.indexOf("{");
    iEnd = url.indexOf("}");
    segment = url.substr(iStart+1,iEnd - iStart-1);
    if (segment[0]=="?"){
      params = segment.substr(1).split(",");
      replace = params.map(function(param){
        var value = document.getElementById(data["groups"][iG]["routes"][iR]['methods'][iM]['name'].replace(/\s/g,"") + "_" + param).value;
        return param + "=" + value;
      }).join("&");
      url = url.replace("{"+segment + "}","?" + replace);
    } else {
      param = segment;
      id = document.getElementById(data["groups"][iG]["routes"][iR]['methods'][iM]['name'].replace(/\s/g,"") + "_" + param);
      replace = id?id.value:id;
      url = url.replace("{"+segment + "}",replace);
    }
  }
  url = encodeURI(url);
  ...
  var request = {
    url: host + url,
    headers:  headers,
    method: data["groups"][iG]["routes"][iR]['methods'][iM]['method']
  }

  if (!req['body']) {
    delete data["groups"][iG]["routes"][iR]['methods'][iM]["dialog"][0]["request"]["body"];
  } else {
    request.type =req['type'] || "text/plain";
    request.data = req['body'].join("\n");
  }
  sp3Request(request,function (response){
    if (response){
      data["groups"][iG]["routes"][iR]['methods'][iM]["dialog"][0]["responses"][0]["status"] =
        response["status"].toString() + (response["statusText"]?(" (" + response["statusText"] + ")"):'');
      data["groups"][iG]["routes"][iR]['methods'][iM]["dialog"][0]["responses"][0]["type"] =
...
var sp3Request = function(request,responseFn){
  ...
  if (request.data){
    obj["mimetype"] = request.type;
    obj["data"] = request.data;
  };
  try{
    majaX(obj, function(results,response) {
      responseFn(response);
    }, function(results,response) {
      if (response['status']==0) {
        data['error'] = ["Request failure","","No response"]
        responseFn(false);
      } else {
        responseFn(response);
      }
    ...
    data['error'] = ["Server connection failed!","",e];
    responseFn(false);
  }
}
